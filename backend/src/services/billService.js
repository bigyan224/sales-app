...
function docToBill(doc) {
  return {
    id: String(doc._id),
    name: doc.name,
    bsDate: doc.bsDate,
    adDate: doc.adDate ?? null,
...
export async function getBills(options = {}) {
  ...
  const [docs, total] = await Promise.all([
    BillModel.find(query).sort({ updatedAt: -1 }).skip(offset).limit(limit).lean(),
    BillModel.countDocuments(query),
  ]);

  return { bills: docs.map(docToBill), total, limit, offset };
}

export async function getBillById(id) {
  const doc = await BillModel.findOne({ _id: id, deletedAt: null }).lean();
  return doc ? docToBill(doc) : null;
}
...
export async function batchSync(items) {
  ...
  if (active.length > 0) {
    ...
    const existing = await BillModel.find({ _id: { $in: ids } }).lean();
    ...
    if (operations.length > 0) {
      await BillModel.bulkWrite(operations, { ordered: false });
    }
  }

  if (tombstones.length > 0) {
    await BillModel.deleteMany({ _id: { $in: tombstones.map((t) => t.id) } });
...
export async function deleteBillById(id) {
  const result = await BillModel.deleteOne({ _id: id });
